...
public abstract class Page {
	...
	public void switchToAlert(String action) {
		try{
		String arg = action.toLowerCase();
		Alert alert = browser.switchTo().alert();
		System.out.println("The alert pop-up's text is : " + alert.getText());
		if (arg.equals("yes")) {
			alert.accept();
		} else
			alert.dismiss();
		}catch(Exception ex){
			Common.testStepFailed("Unable to switch to alert pop up Exception is->"+ex.getMessage());
		}
	}
